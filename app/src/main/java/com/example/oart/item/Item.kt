...
class Item(private val document: QueryDocumentSnapshot, private val user: String) {
    ...
    val speed = ((floor((document.data["speed"] as Double)))).toString().padStart(2,'0') + ":" + (floor(((document.data["speed"] as Double) % 1)*60)).toString().padStart(2,'0')
    val distance = ((((document.data["distance"] as Double) * 100).roundToInt() /100) / 1000).toDouble().toString()
